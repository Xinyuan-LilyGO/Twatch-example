...
bool BMA::begin()
{
    ...
    bma423_select_platform(BMA423_WRIST_CONFIG, &_dev);
...
void BMA::disalbeIrq()
{
    bma423_map_interrupt(BMA4_INTR1_MAP, BMA423_STEP_CNTR_INT | BMA423_TILT_INT | BMA423_WAKEUP_INT, BMA4_DISABLE, &_dev);
}

void BMA::enableIrq()
{
    bma423_map_interrupt(BMA4_INTR1_MAP, BMA423_STEP_CNTR_INT | BMA423_TILT_INT | BMA423_WAKEUP_INT, BMA4_ENABLE, &_dev);
}
...
void BMA::attachInterrupt()
{
    ...
    rslt |= bma423_feature_enable(BMA423_ANY_MOTION, BMA4_ENABLE, &_dev);
    rslt |= bma423_feature_enable(BMA423_ACTIVITY, BMA4_ENABLE, &_dev);

    rslt |= bma423_step_counter_set_watermark(100, &_dev);

    // rslt |= bma423_map_interrupt(BMA4_INTR1_MAP, BMA423_TILT_INT | BMA423_STEP_CNTR_INT | BMA423_WAKEUP_INT, BMA4_ENABLE, &_dev);
    ...
    // rslt |= bma423_map_interrupt(BMA4_INTR1_MAP, BMA423_WAKEUP_INT, BMA4_ENABLE, &_dev);
    // rslt |= bma423_map_interrupt(BMA4_INTR1_MAP, BMA423_ANY_NO_MOTION_INT, BMA4_ENABLE, &_dev);
    // rslt |= bma423_map_interrupt(BMA4_INTR1_MAP, BMA423_ACTIVITY_INT, BMA4_ENABLE, &_dev);
    ...
    bma423_get_remap_axes(&remap_data, &_dev);

    Serial.printf("x_axis:%u ,x_axis_sign:%u , y_axis:%u , y_axis_sign:%u ,z_axis:%u ,z_axis_sign:%u \n",
                  remap_data.x_axis, remap_data.x_axis_sign, remap_data.y_axis, remap_data.y_axis_sign, remap_data.z_axis, remap_data.z_axis_sign
                 );
    // ! No.1
    // x_axis:0 ,x_axis_sign:0 ,
    //  y_axis:1 , y_axis_sign:0 ,
    //  z_axis:2 ,z_axis_sign:0
    //! No.2
    remap_data.x_axis = 0;
