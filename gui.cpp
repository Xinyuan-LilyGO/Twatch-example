...
stack<funcCallback> mStack; //Menu bar stack
...
static void event_handler(lv_obj_t *obj, lv_event_t event)
{
    ...
    }  else if (event == LV_EVENT_SHORT_CLICKED) {  //!  Event callback Is in here
        if (obj == menuBtn) {
...
void setupGui()
{
    ...
    //Create wallpaper
    void *images[] = {(void *) &bg, (void *) &bg1, (void *) &bg2, (void *) &bg3,(void*)&WALLPAPER_1_IMG,(void*)&WALLPAPER_2_IMG,(void *)&WALLPAPER_3_IMG };
    lv_obj_t *scr = lv_scr_act();
    lv_obj_t *img_bin = lv_img_create(scr, NULL);  /*Create an image object*/
    srand((int)time(0));
    int r = rand() % 7;
    // lv_img_set_src(img_bin, "P:/bg.bin");  /*Set the created file as image (a red rose)*/
...
void updateBatteryIcon(lv_icon_battery_t icon)
{
    if (icon >= LV_ICON_CALCULATION) {
        int level = axp.getBattPercentage();
            ...
            mbox->create(format, [](lv_obj_t *obj, lv_event_t event) {
                if (event == LV_EVENT_VALUE_CHANGED) {
                    ...
                    if (!strcmp(txt, "Ok")) {
                        ...
                        rtc.setDateTime(info->tm_year + 1900, info->tm_mon + 1, info->tm_mday, info->tm_hour, info->tm_min, info->tm_sec);
...
static void wifi_destory()
{
    ...
    case 1:
        if (list != nullptr) {
            delete list;
            list = nullptr;
            Serial.println("destory list");
        }
        if (gTicker != nullptr) {
            delete gTicker;
            gTicker = nullptr;
            Serial.println("destory gTicker");
        }
        if (kb != nullptr) {
            delete kb;
            kb = nullptr;
            Serial.println("destory kb");
        }
        if (pl != nullptr) {
            delete pl;
            pl = nullptr;
            Serial.println("destory pl");
        }
        sw->hidden(false);
        break;
    //! wifi keyboard
...
static void bluetooth_event_cb()
{
    ...
    Switch::switch_cfg_t cfg[2] = {{"Switch", bluetooth_sw_event_cb}, {"Scan", bluetooth_sw_event_cb}};
    sw = new Switch;
    sw->create(cfg, 2, []() {
        delete sw;
        sw = nullptr;
    ...
    sw->align(bar.self(), LV_ALIGN_OUT_BOTTOM_MID);
    sw->setStatus(0, bluetooth_isConnect());
}
...
static void sd_event_cb()
{
    ...
    // l    istDir(SD, "/", 0);
    Serial.println("SD Card interface no complete....");
    menuBars.hidden(false);
}
 ...
 *
 */
static const char *camera_btns[] = { "Quit", ""};

static void camera_event_cb()
{
    EventBits_t bits = xEventGroupGetBits(g_event_group) & G_EVENT_WIFI_CONNECTED;
    if (bits == pdFALSE) {
        Serial.println("WiFi is not connect");
        create_mbox("WiFi is not connect", sd_mbox_event_cb);
        lv_mbox_add_btns(mbox1, camera_btns);
        return;
    }
    menuBars.hidden(false);
}